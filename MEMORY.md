...
- [🔴🔴🔴打法說明只進不退,舊件不回頭改|0731修正「舊件」定義](feedback_rules_forward_only_no_retro_edit_0730.md) — 規則往前更新,已歸檔memo保持原貌;🔴判準=這份memo還會不會再被送出去?會(退件重送/待補件)→活件要用現行規則重打;不會(已核准歸檔)→舊件不動
- [🔴🔴🔴使用者丟回改好的版本=隱含規則,要逆向抽出](feedback_user_edit_implies_rule_0730.md) — 比對逐字差異→反推通則→兩本打法說明+記憶同步→回測加測項→回報讓他否決;已抽多條memo格式規則(期數換行/檢附看角色不看抬頭);回測116項全PASS
- [🔴🔴🔴簽單抽點規則+「❌未查」不是放行理由(0804定版)](reference_signature_slip_sampling_rule_0804.md) — ≤20張強制逐張核;>20張抽點=MAX(20,N÷20)(15等距+5指定金額槓桿最大);使用者說「全部抽點」不得縮減;🔴有❌未查的件不得進4.核准,M欄要寫「使用者裁示免核」非「未查」
...
## 請款/計價 規則
- [🔴🔴🔴總經理規定:行通表門檻(四大類)|0731加時間界線](feedback_hangtong_existence_gate.md) — 機具/點租工/勞安衛/建材沒登記一律不予計價;🔴🔴115.08起①型查無直接不予請款不再補救(②型不受影響);季別＝出貨/施作月所屬季非計價當季;工具=桌面行通表夾_查行通表.ps1(58項全PASS才可用)
- [🔴🔴請款memo標準打法範本(定版)](reference_billing_memo_standard_template.md) — 第1點帶計價區間+#期數換行不加逗號+代扣拆自辦/代辦(G4全額)+扣款行帶簽名狀態+檢附看角色(請款依據=請款單)+第3點兩項以上一項一行+檢附用通名『列控表』不用廠商抬頭
- [🔴🔴🔴0803問題清單分頁制度+總表已改版無備註欄](feedback_issue_list_sheet_workflow_0803.md) — 使用者用意=規則釐清完要寫進打法說明+記憶(不是只列缺件);A區規則/B區逐案缺件;🔴總表改成「待核/核定計價」兩區無備註欄,五處同步第②處改指向問題清單;找總表列要掃全欄別寫死欄號
- [🔴🔴🔴0803改成正確模式+計價本回測上線](feedback_billing_book_correct_mode_0803.md) — memo不放說明性敘述段;拆自辦代辦就必須有扣款行+簽名句;期數排序只能在同一合約號內比;搬列一律Excel整列Cut→Insert;`_計價本回測.ps1`12測項,動完計價本必跑
...
- [🔴🔴搬進核定計價看「單號層級」+合計列必重算(0804定)](reference_archive_move_zone_rule_0804.md) — 核定=這個單號沒有任何一件在跑;搬區前後總數必須相等;🔴追蹤報表合計列會默默過時,動完備註一律程式逐列重算;141E總表尚未改版待裁示
...
- [🔴🔴🔴每批作業要另寫「本輪專用回測」驗自己(0804)](feedback_own_work_backtest_caught_errors_0804.md) — 既有三支只驗通則不驗本輪具體宣稱,全綠≠做對;0804寫45測項第一次跑就抓到三個錯(行通表門檻沒過卻列檢附/檢附列了卻沒放檔/測項太鬆分不出行別);測項要能指出「該改哪一格」
...
- [🔴🔴請款太晚要「確實稽核」(0731升級)](feedback_late_billing_alert.md) — 計價月=核定當月;呈核日=處理當天;施作距計價>3個月打延遲句(以起始月判);🔴每件進來就算月差,超過3個月要獨立成段列出,不可留空
...
- [內部碳費基金修改單範式](reference_carbon_fee_change_order.md) — 非業變/純預算調整/淨0;工程專案主管◎決;141A-154首例71,764
- [入預算作業流程](reference_budget_entry_workflow.md) — 141E主檔分頁顏色標狀態;主檔複製加MMDD前綴
- [141A-136齊昇送採購議價](project_change_order_141A136_0624.md)
...
- [🔴PowerShell變數名不分大小寫($D=$d)](reference_powershell_variable_case_trap.md) — 靜默覆寫陣列→整批數字錯還不報錯;規則=用長變數名/迴圈索引每圈重設/數字先做量級常識檢查;請款Excel掃描腳本一樣會中
- [🔴🔴Excel COM中文locale三個轉型雷(0804)](reference_excel_com_locale_cast_traps_0804.md) — NumberFormat吃本地字串'General'丟1004要寫'G/通用格式';指派@格式欄要先[string]轉字串;合計列金額要用.Formula寫非.Value2;判斷方向=InvalidCastException在PS層/1004是Excel拒絕
- [🔴🔴中文字串兩個隱蔽踩雷(0804)](feedback_chinese_string_powershell_traps_0804.md) — ①絕不手算\uXXXX escape,中文一律用原字寫UTF-8資料檔讀②PS5.1讀無BOM的.ps1當ANSI→「missing the terminator」不是語法錯,執行前補UTF8 BOM;批次動Excel前先跑名稱存在性預檢
...
## 歸檔/資料夾
- [工地歸檔規則](reference_site_archive_convention.md) — `Downloads\工地\<編號.案場>\`;請款進2.請款單\<月份>、修改單進4.修改單
- [🔴Downloads桶名+重複夾清理](project_download_staging_dedup_workflow.md) — 桶=0.5採購議價/1.送出待簽核完成歸檔/2.退件/3.明天整理/4.核准;歸檔指令含「查重複+刪舊留新」
- [問題案夾名加🔴標註;歸檔=清標註再搬](feedback_problem_case_folder_with_note.md) / [歸檔前先放整理夾](feedback_stage_in_downloads_before_archive.md) / [歸檔指令別重翻舊txt](feedback_archive_command_no_reverify.md) / [🔴改memo要同步整列](feedback_row_sync_and_folder_truth_0724.md)
...
## 進行中批次（新→舊）
- [🔴0804:安達#1 Codex抓到人數閘門漏洞+Opus獨立覆核成立](project_billing_anda1_codex_compare_0804.md) — 金額三層閉合；133人次僅132命中(115.05.01請款郭志祥、門禁卻是陳明得)+24人次只刷出無刷進不能證明8小時；Opus重寫程式獨立驗證與Codex一致，可信。狀態＝數量閉合、1人次身分待釐清
- [🔴0804:平安#15暫緩,專案付款金額誤植+重複派工待確認](project_billing_pingan15_0804.md) — 141A-SC009052;專案付款申請代辦誤寫少632,772;派工明細疑重複698;請款區間誤寫06/31;扣墊款63,604已簽但memo漏列;115.07行通表缺級距應走三家比價非直接不予請款;總經理核決
- [🔴0803:8件全審完待裁示+總表改版+38題規則沉澱清單](project_billing_batch_state_0803.md) — 翔博#4/安達#1(🔴0804重審發現5/1姓名錯置)/閎順#1/成駿吊卡#6/欣生#2/雋凱#2/翔盛#8/和浚#1;錢的總帳淨+215,272少付;🔴38題待裁示在計價本問題清單分頁,下次接手先讀別重問
- [0731:9件全審完待覆核+7月全局盤點](project_billing_batch_state_0731.md) — 141A八件+141E捷盛#1,全部歸夾→查核記錄→計價本→總表→回測全PASS,等放行;🔴7月67件盤點抓到14件報表備註空白;祥竹發現預算缺口742,050;吉發#1/捷盛#1卡行通表①型真卡
...
- [0730下午:7件(上展議價後/三菱/聖陸/七祥/移工/曹新泰/承洋)](project_billing_batch_state_0730pm.md) — 上展/三菱/聖陸/七祥/曹新泰已定版;承洋#4放行;🟠移工#2未解(薪資明細/開辦費/代扣差額)+成控表VLOOKUP抓空值已修正
...
- [0728下午收尾:6案+總表已重排](project_billing_batch_state_0728pm.md) — 呂發#7退件、大象#1適燁#1竣葦#9已送出待歸檔、成駿#11竟元#1暫緩;🔴放行≠問題消失(化糞池歸項/翔博架/竣葦超約/成駿預算差641萬);總表按單號重排
...
- [歷史批次現況(0713~0727,多已結案)](project_billing_batch_state_0727.md) — 另見 project_billing_batch_state_0724 / _0723 / _0721 / _0720 / project_billing_0717_batch / _0715 / _0713 / project_billing_chengjun_crane10_0714 / project_billing_chengjun_crane_0623 / project_billing_pingan_0708 / project_taini_2_pending_recheck / project_tracking_item_backfill_0721
- [出差報帳作業](reference_travel_reimbursement.md) — 彙整表在`Downloads\公司\出差報帳\`;系統固定值;應稅免稅分開;本期4趟8,915待送單

## 投資
- [持倉與觀察名單](portfolio_watchlist.md) — 台股2330+00981A(🔴3189景碩已賣出,7/30使用者確認;我連兩天誤報「停損未執行」=拿舊持倉當現況),美股QQQM+NVDA
...
- [🔴🔴🔴廣池崩盤回測0730(250檔×10年×6事件)](feedback_crash_widepool_backtest_0730.md) — 跌越深反彈越大五組單調(最深組fwd60+17.32%/勝82.8%);GAP篩(錯殺vs陷阱)全市場同步崩時會失效;半導體反彈最弱;2022-06型會翻轉結論;鐵則=一律用還原股價
...
- [🔴🔴金融股回測0730:避震器不是加速器](feedback_crash_backtest_financial_vs_0050_0730.md) — 金融股贏0050的三年全是0050漲不動的年份;崩盤後0050 fwd60+11.67% > 金融股+6.27%;🔴唯一例外2022-06(0050唯一為負)銀行全正壽險證券全負;🔴0050台積電權重59.92%對使用者是加倉不是分散
...
- [🔴🔴🔴崩盤的時間維度(0730,n=6正規事件偵測)](feedback_crash_time_to_bottom_0730.md) — 0050含息回本比加權指數快一倍(286.5天vs637天),講回本時間一律用0050;🔴「用歷史中位推落底日」已證偽不准再算日期;跌深與回本速度無關
- [🔴🔴題1必須逐檔查個股新聞不只族群](feedback_thesis1_needs_stock_specific_news.md) — 0730廣達GDS募資712億稀釋=個股利空我漏查,誤列模式A候選吃到停損-9.9%;定案前每檔單獨跑「個股名+近三日」查增資/處置/質押/抽單;跌最深≠可以接(被動元件DD60-58~-69%但題1不過)
...
- [🔴TWSE當日資料抓法+限流雷](reference_twse_api_same_day_data.md) — 盤後當日**不能**用openapi STOCK_DAY_ALL(會靜默停在前一交易日且無日期欄);改RWD MI_INDEX指定日期;BWIBBU的PE是第3欄不是第4欄;TWSE 200+次後回307靜默擋、Yahoo回429;全市場DD60用60天MI_INDEX面板算(~88次)別逐檔打Yahoo
...
- [選股框架實證審核完結](project_stock_framework_refactor.md) / [選股對帳紀錄制度](project_stock_track_record.md) / [模擬倉(已停用)](project_paper_trading.md) / [選股與操作邏輯](stock_selection_logic.md) / [課程教材來源](reference_trading_course_source.md)
...
- ⚠️會過期快照:[🔴0730崩盤現況(最新)](project_market_crash_0730_state.md) — 7/30收39,933破4萬、反彈失敗日非止穩日;金融保險最強/電腦週邊最弱;廣達停損+長榮航降評;決定不動第二批;待辦80萬配置試算 / [717崩盤→722已V轉](project_market_crash_0717_bear_state.md) / [AI七層供應鏈0716](project_ai_chain_signal_snapshot_0716.md)
