...
- 🔴🔴 **每月汰舊一次**（0827 定）：`memory-health.ps1` 在每月第一次開場會噴一行提醒（stamp 檔 `memory\.monthly-purge-stamp` 記已做的月份）。看到那行就**直接跑** `Downloads\agent\計價回測工具\_記憶月度汰舊.ps1`，照報表 A~D 段處理完，把 `yyyy-MM` 寫進 stamp 檔蓋章。不要問他要不要做。
  ...
  - 收尾**兩支都要跑**（-Ref 都給壓縮前的 commit）：
    ① `_記憶壓縮回測.ps1 -OnlyRules` 驗硬 token（數字/條號/檔名/百分比），規則層必須 0 缺失
    ② `_記憶刪規則偵測.ps1 -Threshold 0.45` 驗**純中文規則句有沒有整條消失**——把舊版每條規則句拿去現在的全庫找最像的一行，太不像就報。**噴出來的要逐條看，不是看它綠不綠**：索引行被我壓短、案例句本來就該掉，都會上榜；只有「以後還要照做的規則現在全庫查無」才要補回手冊。0827 這輪 89 條裡挑出 2 條真的掉了（0810 預算段vs合約段單價、0813 memo第2點assert總額），已補回。
...
- 🔴 回測的**已知盲區（0827 已補上工具，但仍要人工判讀）**：`_記憶壓縮回測.ps1` 只比對硬 token，**擋不住純中文敘述的規則被整條刪掉**（0806 那兩條就是回測 PASS 但規則掉了，人工才發現）。0827 新增 `_記憶刪規則偵測.ps1` 專門補這個洞，用字元 bigram 找「舊規則句在現在全庫的最佳相似度」。🔴 它會有雜訊（HANDOFF.md 已排除；索引行壓縮、案例句本來就該掉），**輸出是待判清單不是紅綠燈**，要逐條判「這是規則還是當時的現況」。

## 🔴 收尾＝驗收不是產出（0827 定）
- 使用者打「收尾」要的是**確認今天安排的事有沒有到位、重點有沒有記起來**，不是要我再生一批東西。收尾預設**不新增記憶檔**；有新規則就寫進既有手冊，沒有就明講「這次沒有新規則」。
- 判準：只有「他糾正我」或「他改了我的東西」才算新規則；我自己覺得值得記的進度、心得、快照，一律不寫。
- HANDOFF.md **只留最近 5 段**，第 6 段起收尾時直接刪（0827 從 88,045 bytes／36 段砍到 13,308 bytes／5 段；舊內容 git log 可取回）。
- 病根：只進不出的寫入流程，最後一定變垃圾場。批次檔一天一本（0827 已併成 project_open_items.md）和 HANDOFF 無限追加，是同一個病的兩個出口。
...
## 回測測項
- T0 🔴收尾時有沒有為了交差開新記憶檔？HANDOFF 有沒有超過 5 段？
