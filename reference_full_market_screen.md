...
## 🔴🔴 零、兩種模式(2026/07/03使用者定，先試兩種；`Downloads\agent\計價回測工具\選股說明.txt`「零」段同步)
...
## 🔴(2026/06/22確認)雲端容器(claude.ai/code網頁/手機) vs 電腦本機CLI，網路能力不一樣
雲端/claude.ai/code的session跑在隔離容器，**有網路白名單，Bash curl連TWSE/Yahoo/Google直接被擋(Host not in allowlist)，WebFetch對外站台也常400~403**；WebSearch能用但這種環境下偶爾會自相矛盾/疑似編數字，中小型個股級資料尤其不可信。
**這台Windows主機本機跑的Claude Code CLI不受這個容器限制，網路是電腦原生網路，理論上抓資料正常。**
→ 如果在雲端/手機環境收到「幫我選股」，先測一次WebFetch/curl，卡住就誠實講清楚(別硬編數字)，並建議使用者改在電腦本機那邊跑選股。
...
## 🔴🔴(2026/07/09使用者點名補，鐵則)國際儀表板兩個硬缺口
1. **台指期必列入固定欄位**：比美股四大更直接反映「今晚夜盤/明天台股開盤」方向。抓法：WebFetch `https://tw.stock.yahoo.com/future/futures.html?fs=WTX%26`，抓近月合約報價/漲跌%/時間戳（標明日盤08:45-13:45或夜盤15:00-隔日05:00）；算法=台指期現價−今日加權收盤=領先程度。cnyes期貨頁常404，抓不到就標未取得別編。
2. **查新聞一定先看發布時間再引用**：新聞網站「熱門排行/焦點」榜常把舊文章跟即時新聞混排，只看標題會把舊聞當即時頭條講。實例（2026/07/09）：把7/7的「美軍空襲伊朗」舊聞誤當7/9即時頭條，被使用者抓包糾正。每則引用都要標「發布於M/D」，標題沒日期就去內文找時間戳，抓不到明確日期就不敢說是「今天」頭條，改用WebSearch配明確日期關鍵字查證而非熱門排行頁。
兩點已同步寫進`Downloads\agent\計價回測工具\選股說明.txt`「三、大盤儀表板」段。
