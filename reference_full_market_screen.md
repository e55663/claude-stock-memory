...
🔴🔴**執行態度(2026/06/17使用者再強調)**：他一打「幫我選股」就**直接把所有資料全跑出來**，不要先問「要哪個選項/要不要等」、不要丟 1/2/3 menu。**有其他建議＝放在「跑完所有資料之後」再補在最後**，不能用建議/提問取代跑資料。籌碼(T86)若白天還沒出，就照常全跑、在輸出裡標「籌碼傍晚才到」，不要因此停下來問他。
