...
🔴🔴(2026/7/15第二輪調整,原因見上面「實測結果=無效」)使用者發現一個額外的計時漏洞並修正:**cron本身只會晚不會早**(從list API `last_fired_at`實測驗證:過去四次全部晚2~6分鐘,沒有一次提早),但如果緩衝抓太小(原本11:01/16:02/21:03只抓1-3分鐘),遇到上一輪本身也延遲(如16:00那輪實測晚6分),下一輪反而會在「舊視窗還沒真正到期」時就觸發,等於白打、沒建立新錨點,而且這個誤差會一輪一輪往後疊加。**修正=緩衝加大到10分鐘、每輪間隔拉開成5小時10分**,四個錨定改成:
  - 額度視窗錨定 05:00 = `trig_01N6vgMEk1HayQUB5eTo3T7W` cron `0 21 * * *`
  - 額度視窗錨定 10:10 = `trig_01GKzSNYyQyapCtdeWa1nPJ5` cron `10 2 * * *`
  - 額度視窗錨定 15:20 = `trig_013muPWXX98pNxogjV54Lkev` cron `20 7 * * *`
  - 額度視窗錨定 20:30 = `trig_01E8ZFChRQkTL5MHWRbPXUFX` cron `30 12 * * *`(跟晚間待辦提醒20:30同分鐘但不同trigger,不衝突)
  設計上20:30→隔天05:00中間空8.5小時(遠超過5hr視窗),等於每晚自動歸零,單日內某一輪誤差不會累積過夜。**但這只解決「緩衝不夠」這個子問題,不解決「雲端routine跟互動可能根本不同一個額度桶」這個更根本的懷疑——要等實際觀察reset時間才知道有沒有真的解決,還沒驗證。**
