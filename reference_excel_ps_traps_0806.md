...
- 🔴🔴🔴 **115.09.01 這次沒活下來，而且 Assert-WriteLanded 沒抓到**（東和#6）：13:52:53 我寫入 141E 計價本補 memo 換行 → 339,998 bytes、`Assert-WriteLanded` 回 OK；同一分鐘檔案被回存成 339,961 bytes，內容退回修正前，我那次修正整份消失（同時 8月追蹤報表被寫成 97,489 bytes、承辦人由我填的「待補」變成他填的「林逸宏」）。**病根＝`Assert-WriteLanded` 只證明「我寫進去了」那一刻，不證明「之後沒被蓋回去」**；而且他回存的秒數跟我幾乎同秒，`LastWriteTime` 看起來就是我的那次，光看時間戳分辨不出來。
  🔴 **加一道收尾驗證：每輪回報「完成」之前，對這輪寫過的每個桌面 xlsx 重讀一次關鍵格（不是只看時間戳，要比對內容或 byte 大小和我寫入時的值）**。大小對不上＝被蓋掉，重補。這也是為什麼一次性本輪回測要真的再跑一次，不能只跑一次就收工——0901 就是靠 `_計價本回測` T3 重跑才發現修正不見了。
