...
metadata: 
  ...
  modified: 2026-07-31T03:07:30.050Z
---
...
## 🔴 用資料檔餵中文進 PowerShell 的兩個雷（115.07.31 打 7 件計價本時踩到）

要把中文 memo/註記寫進 Excel，正解是**「腳本不含中文、中文全放 UTF-8 資料檔，腳本讀檔」**。但這招有兩個坑：

1. **`.ps1` 檔沒有 BOM → PowerShell 5.1 當 ANSI 讀，腳本裡的中文字面值直接變亂碼**。
   症狀：`"141A、"` 變成 `"141A??"`，寫 Excel 公式時丟 `HRESULT: 0x800A03EC`。
   解法：Write 工具產生 `.ps1` 後，**一定要補 UTF8 BOM**：
   ```powershell
   $t=[IO.File]::ReadAllText($p,[Text.Encoding]::UTF8)
   [IO.File]::WriteAllText($p,$t,(New-Object Text.UTF8Encoding($true)))
   ```
2. **資料檔的欄位分隔符絕不能用 `~`、`-`、`|` 這種會出現在內容裡的字元**。
   我用 `~` 當分隔符，結果昇峰註記裡的「90~100T全吊」「70~80T」、翔博的「132~150元」、
   吉發的「3,000~3,500」把整段 M 欄註記**從中間切斷**，寫進 Excel 只剩片段，
   而且 **Excel 不會報錯**——要靠「寫入長度 vs 讀回長度 vs `-ceq` 比對」才抓得到。
   解法：用 `#|#` 這種複合分隔符（`-split [regex]::Escape('#|#')`），
   換行用 `||` 佔位再 replace；**寫完一定回讀比長度＋`-ceq`**。
   噸位區間、金額區間、日期區間在請款註記裡很常見，這條每次都會遇到。

另外兩個同批踩到的 COM 雷：
- `$xl.CutCopyMode=0` 在 PS 會丟列舉型別錯誤 → 改
  `$xl.GetType().InvokeMember("CutCopyMode","SetProperty",$null,$xl,@(0))` 包 try/catch。
- 複製既有列往下堆時，若來源儲存格是**文字格式 `@`**，`.Formula` 會被當純文字存進去、公式不運算
  （昇峰 B5 中招）。設公式前先把該格 `NumberFormat` 改回 `G/通用格式`。

相關 [[feedback_crash_time_to_bottom_0730]]、[[feedback_ps_chinese_literal_encoding]]、[[reference_excel_com_scan_pitfalls]]、[[reference_twse_api_same_day_data]]、[[feedback_never_mix_bash_powershell_file_ops]]。
